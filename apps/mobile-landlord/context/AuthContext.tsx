...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadAuthState = async () => {
    try {
      ...
      const storedActiveRole = await AsyncStorage.getItem(STORAGE_KEY_ACTIVE_ROLE);
  ...
  const login = async (
    ...
    try {
      ...
      const mergedUser: User = {
        ...newUser,
        roles,
        needsOnboarding: identity?.needsOnboarding ?? newUser.needsOnboarding ?? false,
        activeRole,
      };
  ...
  const logout = useCallback(async () => {
    try {
      ...
      if (currentToken) {
        await createServerTrpcClient(currentToken).auth.logout.mutate().catch(() => null);
      }
