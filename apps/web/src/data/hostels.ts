...
export const baseHostels: Hostel[] = [
  {
    slug: 'greenfield-boys-hostel',
    name: 'Greenfield Boys Hostel',
    shortDescription:
  ...
  {
    slug: 'sunrise-ladies-hostel',
    name: 'Sunrise Ladies Hostel',
    shortDescription:
      'Secure women-only hostel with hygienic food, housekeeping, and transport access.',
    location: 'Bengaluru',
...
// Generate more hostels for demo
export const hostels: Hostel[] = Array.from({ length: 12 }, (_, i) => {
  const base = baseHostels[i % baseHostels.length];
  return {
    ...base,
    slug: `${base.slug}-${i + 1}`,
...
export const hostelLocations = Array.from(new Set(hostels.map((hostel) => hostel.location))).sort();

export function getHostelBySlug(slug: string) {
  return hostels.find((hostel) => hostel.slug === slug);
}
