    ...
    <nav className="bg-card fixed right-0 bottom-0 left-0 z-50 flex h-16 items-center justify-between border-t px-4 sm:px-6 md:hidden">
      {MobileNavButtons.map((btn) => (
        <button
          key={btn.label}
          onClick={btn.onClick}
          className={`flex w-16 flex-col items-center gap-0.5 rounded-xl p-2 transition-colors active:scale-95 ${btn.active ? 'bg-orange-500/5 text-orange-600' : 'text-muted-foreground'}`}>
          <btn.icon size={24} />
