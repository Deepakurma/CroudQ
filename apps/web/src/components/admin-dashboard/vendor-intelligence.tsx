    ...
    cell: ({ row }) => (
      <div className="flex flex-col">
        <span className="font-semibold sm:font-bold">
          {row.getValue('vendors')}{' '}
          <span className="text-muted-foreground/70 ml-0.5 font-medium">vendors</span>
        </span>
      </div>
    )
  },
...
export function VendorIntelligence({ data: inputData, isLoading }: VendorIntelligenceProps) {
  return (
    <div className="flex flex-col gap-2 p-4 sm:gap-3">
      <h1 className="text-sm font-semibold tracking-widest uppercase">
        City wise vendor distribution
      </h1>
