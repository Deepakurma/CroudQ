...
interface VendorStats {
  ...
  convertedVendorAccounts: number;
}

type RawVendor = Omit<Vendor, 'createdAt' | 'startDate' | 'endDate'> & {
  createdAt: string;
  startDate: string;
  endDate: string;
};
        ...
        <div className="flex w-[130px] flex-col gap-3 whitespace-normal">
          ...
          {openRoomsDialog && (
            <CustomDialog
              ...
              title={`Rooms & Pricing — ${row.original.vendorName}`}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isPending && (
              <Button
                ...
                onClick={() => alert(`Notification sent to ${row.original.vendorName}`)}>
                <BellRing size={12} />
                Notify
              </Button>
            )}
...
function VendorRowActions({
  ...
  vendor: Vendor;
  onToggleFreeze: (vendor: Vendor) => void;
}) {
  const freezeLabel = vendor.isFrozen ? 'Unfreeze Vendor' : 'Freeze Vendor';
  const dialogTitle = vendor.isFrozen ? 'Unfreeze Vendor' : 'Freeze Vendor';
  const dialogDescription = vendor.isFrozen
    ? `Do you want to unfreeze ${vendor.vendorName}?`
    : `Do you want to freeze ${vendor.vendorName}? Frozen accounts cannot access vendor operations.`;
      ...
      <DropdownMenuContent align="end">
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <DropdownMenuItem
              variant={vendor.isFrozen ? 'default' : 'destructive'}
          ...
          <AlertDialogContent>
            ...
            <div className="flex justify-end gap-2">
              ...
              <AlertDialogAction asChild>
                <Button onClick={() => onToggleFreeze(vendor)}>
...
export default function page() {
  ...
  const [vendorStats, setVendorStats] = useState<VendorStats>({
    totalVendorAccounts: 0,
    convertedVendorAccounts: 0
  });
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      setIsLoading(true);
      try {
        const payload = (await trpcClient.admin.listVendors.query({
          q: debouncedSearch || undefined
        })) as RawVendor[] | VendorsApiResponse;

        const payloadVendors = Array.isArray(payload)
          ? payload
        ...
        const mapped = payloadVendors.map((item) => ({
          ...item,
        ...
        const seenVendorAccounts = new Set<string>();
        const deduped = mapped.filter((item) => {
          const vendorAccountKey = item.userId || item.id;
          if (seenVendorAccounts.has(vendorAccountKey)) return false;
          seenVendorAccounts.add(vendorAccountKey);
          return true;
        });

        setVendors(deduped);
        setVendorStats({
          totalVendorAccounts: payloadStats?.totalVendorAccounts ?? deduped.length,
          convertedVendorAccounts: payloadStats?.convertedVendorAccounts ?? deduped.length
        });
      } catch {
        setVendors([]);
        setVendorStats({
        ...
        toast.error('Could not fetch vendors right now. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleToggleFreeze = async (vendor: Vendor) => {
    try {
      await trpcClient.admin.setVendorFreeze.mutate({
        id: vendor.id,
        isFrozen: !vendor.isFrozen,
        freezeReason: !vendor.isFrozen
      ...
      toast.success(vendor.isFrozen ? 'Vendor unfrozen' : 'Vendor frozen');
    } catch {
      toast.error('Could not update vendor status right now. Please try again.');
    }
  };

  const columnsWithActions = useMemo<ColumnDef<Vendor>[]>(() => {
    return columns.map((column) => {
      if (column.id !== 'actions') return column;
      return {
        ...column,
        cell: ({ row }) => (
          <VendorRowActions vendor={row.original} onToggleFreeze={handleToggleFreeze} />
        )
      };
    });
  }, [handleToggleFreeze]);
  ...
  const filteredData =
    filter && filter !== '' ? vendors.filter((item) => item.status === filter) : vendors;

  const cards = useMemo<PulseCardProps[]>(() => {
    const pendingRenewals = vendors.filter((vendor) => vendor.status === 'Pending Renewal').length;
    const totalCapacity = vendors.reduce((sum, vendor) => {
      return (
        sum +
        vendor.roomTypes.reduce((capacitySum, roomType) => {
          return capacitySum + roomType.capacity * roomType.rooms;
        }, 0)
      );
    }, 0);
    const convertedVendorAccounts = vendorStats.convertedVendorAccounts;
    const totalVendorAccounts = vendorStats.totalVendorAccounts;
    const conversionRate =
      totalVendorAccounts > 0
        ? Math.round((convertedVendorAccounts / totalVendorAccounts) * 100)
        : 0;

    return pulseData.map((card) => {
      if (card.label === 'Total Vendors') return { ...card, value: totalVendorAccounts };
      if (card.label === 'Pending Renewals') return { ...card, value: pendingRenewals };
      if (card.label === 'Conversion Rate')
        return {
          ...card,
          value: `${conversionRate}%`,
          sub: `${convertedVendorAccounts}/${totalVendorAccounts} vendor accounts created at least one property`
        };
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="box-border flex w-full flex-col gap-2 overflow-hidden p-4 sm:gap-3">
        <h1 className="text-sm font-semibold tracking-widest uppercase">All vendors</h1>
        <DataTable
