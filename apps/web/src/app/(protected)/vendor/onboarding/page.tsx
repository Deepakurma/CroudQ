...
function OnboardingScreenContent() {
    ...
    const loadEditData = async () => {
      ...
      try {
        ...
        if (!properties.length) {
          ...
          router.replace('/vendor/property');
          return;
        }
        ...
        const nextType = allowedHostelTypes.includes(details.type as OnboardingFormData['type'])
  ...
  const submitEditPropertyUpdate = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['vendor-property-summary'] });
  ...
  return (
    <div className="bg-background text-foreground mx-auto flex min-h-screen max-w-3xl flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Button onClick={() => void nextStep()} disabled={isSubmitting} className="flex-[2]">
