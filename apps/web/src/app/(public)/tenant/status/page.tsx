    ...
    const fetchStatus = async () => {
      try {
        ...
        if (trpcError?.data?.code === 'UNAUTHORIZED') {
          router.replace('/tenant/auth?redirect=%2Ftenant%2Fstatus');
          return;
        }
        if (active) {
          setShouldPoll(false);
              ...
              {data?.status === 'none' || data?.status === 'invited' ? (
                <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-slate-900">
                  <p className="text-base font-medium">
                    You&apos;re currently not joined in any hostel.
                  </p>
                  <p className="text-sm">The hostel you join will appear here.</p>
