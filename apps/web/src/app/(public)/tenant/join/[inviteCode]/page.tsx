...
export default function JoinInvitePage() {
    ...
    const fetchInvite = async () => {
      ...
      try {
        ...
        if (payload?.inviteState === 'expired') {
          router.replace('/tenant/auth?reason=expired&redirect=%2F');
          return;
        }

        if (!payload?.allowSubmission) {
          router.replace('/tenant/status');
          return;
        }
  ...
  const handlePhotoChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // Keep payload size within request body limits.
    if (file.size > 700 * 1024) {
      setErrorMessage('Photo size should be under 700KB.');
      return;
    }
    ...
    reader.onerror = () => {
      setErrorMessage('Could not process selected image.');
    };
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <div className="space-y-2 rounded-xl border p-3">
                    ...
                    {profileImageName ? (
                      ...
                      <p className="text-muted-foreground text-xs">Max size: 700KB</p>
                    )}
                  </div>
                </div>
                <div className="space-y-2">
