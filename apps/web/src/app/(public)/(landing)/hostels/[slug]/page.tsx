...
import { HostelImageCarousel } from '~/components/user-dashboard/hostel-image-carousel';
...
export default function HostelDetailPage() {
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-3 sm:gap-5">
        <Card className="flex w-full flex-1 flex-col justify-center rounded-3xl border shadow-sm">
          <CardHeader className="items-center space-y-3 text-center">
            ...
            <CardTitle className="text-2xl">Loading hostel...</CardTitle>
          </CardHeader>
        </Card>
      </main>
    );
  ...
  const images = hostel.images.length > 0 ? hostel.images : ['/assets/Full-Logo.jpeg'];
    ...
    <main className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-3 sm:gap-5">
      ...
      <section className="overflow-hidden rounded-3xl border shadow-sm">
        <div className="md:hidden">
          <HostelImageCarousel images={images} hostelName={hostel.name} />
        </div>
      ...
      <div className="grid items-start gap-3 sm:gap-5 lg:grid-cols-3">
        <Card className="rounded-3xl border shadow-sm lg:col-span-2">
          <CardContent className="space-y-7">
            ...
            <div className="bg-muted/30 flex items-center justify-between gap-3 rounded-2xl border px-4 py-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm font-semibold lg:text-base">{hostel.phoneNumber}</span>
                </div>
              </div>
              <Button asChild size="sm" className="shrink-0 rounded-xl">
                <Link href={`tel:${hostel.phoneNumber}`} aria-label={`Call ${hostel.phoneNumber}`}>
          ...
          <Card className="gap-3 rounded-3xl border shadow-sm">
            ...
            <CardContent className="text-sm lg:text-base">
              {hostel.sharingTypes.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {hostel.sharingTypes.map((sharingType) => (
